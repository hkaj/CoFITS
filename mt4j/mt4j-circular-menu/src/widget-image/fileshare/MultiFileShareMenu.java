...
public class MultiFileShareMenu extends MTRectangle {
	...
	public MultiFileShareMenu(PApplet applet, Workbench workbench,
			List<Workbench> workbenchs, /*File files[]*/ArrayList<File> files) {
		super(applet, 0, 0);
		this.files = files;
	...
	private void addTickBox(String teamMember, float yPos,
		...
		tickBox.setFillColor(COLOR_NOT_SHARED);
		addChild(tickBox);
		...
		public boolean processGestureEvent(MTGestureEvent ge) {
			if (ge instanceof TapEvent && ((TapEvent) ge).isTapped()) {
				if (shareWithMembers.contains(teamMember)) {
					...
					membersTickBox.get(teamMember).setFillColor(COLOR_SHARED);
				}
			...
			for (File file : files) {
				...
				if (wbH != null && wbH.length > 0) {
					for (WorkbenchHandler handler : wbH) {
						if (handler.getWorkbench().getTeamMember()
								.equals(member)) {
							handler.displayUserInfo(file.getName()
									+ " a été ajouté à votre dropbox");
						}
					}
				}
			}
