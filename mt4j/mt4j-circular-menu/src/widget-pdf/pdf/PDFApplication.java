...
public class PDFApplication extends MenuApplication {
	...
	public void play() {
		...
		if (mtApplication instanceof ProjectBoot) {
			workbenchs = ((ProjectBoot) mtApplication).getProject()
					.getWorkingposts();
		} else if (mtApplication instanceof WhiteboardBoot) {
			workbenchs = ((WhiteboardBoot) mtApplication).getProject()
					.getWorkingPosts();
		} else {
			System.err.println("WORKBENCHS NOT FOUND");
			return;
		}
		AbstractShape component = new PDFChooser(getParentComponent()
