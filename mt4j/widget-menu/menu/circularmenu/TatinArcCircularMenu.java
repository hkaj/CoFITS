...
public class TatinArcCircularMenu extends MTEllipse {
//public static final MTCircularMenuBehaviour BEHAVIOUR_DEFAULT = new MTCircularMenuBehaviour();
	...
	private ArrayList<TatinCircularMenuSegmentHandle> segmentHandles = new ArrayList<TatinCircularMenuSegmentHandle>();
	
	private ArrayList<TatinCircularMenuSegment> segments = new ArrayList<TatinCircularMenuSegment>();
	
	private boolean dirty;
	
    private TatinCircularMenuBehaviour behavior = new TatinCircularMenuBehaviour();
	
	private float innerRadius,outerRadius;
	
	private int aperture;
	private IFont font;
	public TatinArcCircularMenu(PApplet pApplet, Vector3D centerPoint,
			float innerRadius, float outerRadius, int aperture) {
		super(pApplet, centerPoint, outerRadius, outerRadius);
		this.pApplet = pApplet;
		PropertyManager pm = PropertyManager.getInstance();
		//this.innerRadius = pm.getIntProperty(PropertyManager.CIRCULAR_MENU_INNERRADIUS);
		//this.outerRadius = pm.getIntProperty(PropertyManager.CIRCULAR_MENU_OUTERRADIUS);
		...
		this.font = FontManager.getInstance().createFont(this.pApplet, 
				pm.getProperty(PropertyManager.CIRCULAR_MENU_FONT), 
				pm.getIntProperty(PropertyManager.CIRCULAR_MENU_SIZE));
	 ...
	 */
	public TatinCircularMenuSegmentHandle createSegment(String itemText) {
		MTTextArea item = createText(itemText, this.font);
		item.setName("Textarea '" +itemText+ "'");
		return this.createSegment(item);
	}
	public TatinCircularMenuSegmentHandle createSegment(PImage icon) {
		MTRectangle rect = new MTRectangle(pApplet, icon);
		rect.setPickable(false);
		rect.setNoStroke(true);
		return this.createSegment(rect);
	}
	public TatinCircularMenuSegmentHandle createSegment(AbstractShape item) {
		TatinCircularMenuSegmentHandle segmentHandle = this.createSegmentHandle();
	...
	public void setFillColor(MTColor fillColor, int segmentID) {
		this.segments.get(segmentID).setFillColor(fillColor);
		//this.setDirty(true);
	}
	...
	private synchronized TatinCircularMenuSegmentHandle createSegmentHandle(){
		return new TatinCircularMenuSegmentHandle(this.segmentHandleIdCounter++);
	}
	private void createMenu(float innerRadius, float outerRadius){
		...
		TatinCircularMenuSegmentHandle[] segmentHandles = this.getSegmentHandles();
		
		float degreeInc = (float)(aperture/(double)segmentHandles.length);
		
		segments.clear();
		for (int i = 0; i < segmentHandles.length; i++) {
			TatinCircularMenuSegmentHandle currentSegmentHandle = segmentHandles[i];
			...
			TatinCircularMenuSegment segment = new TatinCircularMenuSegment(this, innerRadius, outerRadius, degreeInc, orientationAngleDegrees);
...
//				}
	...
	public TatinCircularMenuSegmentHandle[] getSegmentHandles(){
		return this.segmentHandles.toArray(new TatinCircularMenuSegmentHandle[this.segmentHandles.size()]);
	}
	...
	public TatinCircularMenuBehaviour getBehavior() {
		return behavior;
	}
